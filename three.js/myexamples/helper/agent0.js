...
class Agent {
  constructor(pos, halfSize,name,color) {
  	this.name = name;
    this.pos = pos.clone();
    ...
    this.mesh = agentMesh (this.halfSize, color);
    this.MAXSPEED = 300;
    this.ARRIVAL_R = 7;
    this.size = 5;
    this.score = 0;
  ...
  update(dt) {
	...
	let mind = Infinity,threatening=0;
	let vhat,point,proj;
	let perp,overlap;
	...
	for (let i = 0; i < obs.length; i++) {
		 vhat = this.vel.clone().normalize();
		 point = obs[i].center.clone().sub (this.pos) // c-p
		 proj  = point.dot(vhat);
    ...
    // apply the repulsive force
     vhat = this.vel.clone().normalize();
	 point = obs[threatening].center.clone().sub (this.pos) // c-p
	 proj  = point.dot(vhat);
	...
	if (proj > 0 && proj < REACH) {
		...
		perp.subVectors (point, vhat.clone().setLength(proj));
		let overlap = obs[threatening].size + this.size*2 - perp.length();
    if (overlap > 0) {
